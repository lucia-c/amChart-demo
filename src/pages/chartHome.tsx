        ...
        switch (param) {
            case 'xy':
                return (
                    <Container>
                        <Row>
                            <Col> <Xy {...chartType} data={allData[chartType.id]} changeXYSerie={(serie) => XYseriesType = serie} />
                            </Col>
                            <Col sm={2}>
                                <Stack gap={2}>
            ...
            if (remove) {
                if (XYseriesCount[XYseriesType] > 1) {
                    switch (XYseriesType) {
                        case XYSeriesEnums.Column:
                            newData = {
                                ...current, xy: {
                                    ...current.xy, Column: current.xy.Column.map(series => {
                        ...
                        default:
                            ...
                            newData = {
                                ...current, xy: {
                                    ...current.xy, [XYseriesType]: current.xy[XYseriesType].slice(0, -1)
                                }
                            }
                            break;
                    }
                ...
                switch (XYseriesType) {
                    case XYSeriesEnums.Column:
                        newData = {
                            ...current, xy: {
                                ...current.xy, Column: allData.xy.Column.map(series => series = { ...series, [`value${XYseriesCount.Column}`]: XYseriesCount.Column * 100 })
                            }
                        }
                        break;
                    case XYSeriesEnums.Line:
                        ...
                        newData = {
                            ...current, xy: {
                                ...current.xy, Line: [...allData.xy.Line, {
                                    "year": (parseFloat(String(current.xy.Line[current.xy.Line.length - 1]['year'])) + 1).toString(),
                        ...
                        case XYSeriesEnums.Step:
                            newData = {
                                ...current, xy: {
                                    ...current.xy, Step: [...allData.xy.Step, {
